...
The update notifier is a lightweight **server-side** script (`UpdateNotifier.lua`) that checks whether a newer version of vehiclemanager is available on GitHub. It runs once per resource start and prints a single line to the server console.
...
| `path`      | `vehiclemanager`  | Subdirectory within the repo.                               |
