...
- **Client runtime layer:** `client/client.lua` owns camera activation, follow behavior, hold-control handling, mirror overlay, and cleanup.
...
- **Camera orchestration role:** `client/client.lua`
- **Operational maintenance role (updates):** `UpdateNotifier.lua`

## 4) Call tree (high-level)
1. Resource starts from `fxmanifest.lua`.
2. `Config.lua` exposes the shared config table.
3. `client/client.lua` initializes state, validates config, and starts the two long-running client loops.
4. Player toggles camera path: hold handling only (`toggleControlId` + `toggleHoldMs`) → `startFollowCam()` / `cleanupCamera()` → `updateFollowCam()` or `updateHoodCam()` → mirror draw and control suppression while active.
5. `UpdateNotifier.lua` registers `/ccamupdatecheck`, schedules a delayed startup check, and compares local versus remote version metadata.
...
State ownership is local to `client/client.lua`; no persisted server authority for these states.

## 6) Independent threads
| Thread | Where | Role | Start condition | Loop cadence | End condition |
|---|---|---|---|---|---|
| Virtual mirror poll/render prep loop | `client/client.lua` | Maintains virtual mirror candidate tracking and polling windows | Client script load | Per-frame when active; throttled waits when inactive/no vehicle | Stops only on resource stop |
| Main camera runtime loop | `client/client.lua` | Handles toggle state, control disabling, overlay drawing, and follow-cam updates | Client script load | Per-frame while active; throttled while inactive | Stops only on resource stop |
...
- `client/client.lua` does not emit startup banner prints.
