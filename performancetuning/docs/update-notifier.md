...
The update notifier is a lightweight **server-side** script (`server/UpdateNotifier.lua`) that checks whether a newer version of performancetuning is available on GitHub. It runs once per resource start and prints a single line to the server console.
...
| `path`      | `performancetuning` | Subdirectory within the repo.                               |
